    ...
    <>
      ...
      >
        <div className="mb-3 border-b border-slate-100 pb-3">
          <p className="font-semibold text-slate-900">{appt.service?.name ?? "Rendez-vous"}</p>
          <p className="mt-1 text-sm text-slate-600">{formatTimeRange(appt.starts_at, appt.ends_at)}</p>
          <p className="text-xs text-slate-400">{formatDateTime(appt.starts_at)}</p>
        </div>
        <dl className="space-y-2 text-sm">
          ...
          <div>
            <dt className="text-xs text-slate-400">Praticien</dt>
            <dd className="text-slate-900">{appt.staff?.full_name ?? "—"}</dd>
          </div>
          {appt.resource?.name ? (
            <div>
              <dt className="text-xs text-slate-400">Cabine</dt>
