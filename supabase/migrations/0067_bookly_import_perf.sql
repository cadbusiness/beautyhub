-- Perf du RPC bookly_import_payload : évite les statement_timeout du rôle
-- appelant (anon 3s / authenticated 8s) et ajoute un index fonctionnel pour
-- les lookups clients par email en minuscules, qui déclenchaient un scan.

-- Index fonctionnel (tenant_id, lower(email)) : sans lui, `lower(c.email) = v_email`
-- ne peut pas utiliser clients_tenant_id_email_key (btree sur email brut).
create index if not exists idx_clients_tenant_email_lower
  on public.clients (tenant_id, lower(email))
  where email is not null;

-- Note : pas d'index sur lower(unaccent(...)) parce que public.unaccent n'est
-- pas IMMUTABLE (Postgres refuse). Les tables inst_staff / inst_resources
-- sont petites, le scan reste négligeable.

-- Bump statement_timeout local pour tout appel du RPC. `SECURITY DEFINER`
-- n'override PAS le timeout du rôle appelant : sans ceci le webhook Vercel
-- (qui utilise anonKey) tape 3s puis annule la boucle en plein milieu.
