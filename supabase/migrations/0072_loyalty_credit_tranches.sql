-- Bon fidélité par tranches : 500 points (500 € d’historique) → 17,50 €.
-- Le reliquat reste en progression jusqu’à la prochaine tranche.

alter table public.inst_loyalty_balances
  add column if not exists progress_points integer not null default 0
  check (progress_points >= 0);

alter table public.inst_loyalty_programs
  add column if not exists credit_threshold_points integer not null default 500
  check (credit_threshold_points >= 1);

comment on column public.inst_loyalty_balances.progress_points is
  'Points cumulés (1 € dépensé = 1 point) vers la prochaine tranche.';
comment on column public.inst_loyalty_programs.credit_threshold_points is
  'Seuil en points pour convertir un bon. 500 = dès que l’historique atteint 500 €.';
...
revoke all on function public.inst_loyalty_add_progress(uuid, uuid, uuid, integer, text, uuid, text, text) from public;
grant execute on function public.inst_loyalty_add_progress(uuid, uuid, uuid, integer, text, uuid, text, text) to authenticated;
