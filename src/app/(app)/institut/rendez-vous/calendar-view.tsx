    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="min-w-40 text-center text-sm text-slate-600 dark:text-slate-300">
            {mode === "week_staff"
      ...
      {columns.length === 0 ? (
        ...
        <div className="overflow-x-auto rounded-xl border border-slate-200 dark:border-slate-800">
          ...
          >
            <div className="border-b border-r border-slate-200 bg-slate-50 p-2 dark:border-slate-800 dark:bg-slate-900" />
            {columns.map((c) => (
              <div
                key={c.id}
                className="border-b border-r border-slate-200 bg-slate-50 p-2 text-center text-sm font-medium dark:border-slate-800 dark:bg-slate-900"
              >
                <span
            ...
            <div className="relative border-r border-slate-200 dark:border-slate-800">
              {hours.map((h) => (
                <div
                  key={h}
                  className="border-b border-slate-100 pr-2 text-right text-xs text-slate-400 dark:border-slate-800"
                ...
                {apptsForColumn(col.id).map((a) => {
                  ...
                  return (
                    <button
                      ...
                      className="absolute left-1 right-1 overflow-hidden rounded-md px-1.5 py-0.5 text-left text-xs text-white shadow-sm"
                      style={{
                        top,
                        height,
                        backgroundColor: bg,
      ...
      <form action={saveDetails} className="space-y-2">
        ...
        <label className="block text-xs text-slate-500">
          ...
          <select
            ...
            className="mt-1 h-9 w-full rounded-lg border border-slate-300 px-2 text-sm dark:border-slate-700 dark:bg-slate-900"
          >
